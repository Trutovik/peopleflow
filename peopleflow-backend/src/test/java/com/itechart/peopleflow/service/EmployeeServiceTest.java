...
class EmployeeServiceTest {
  ...
  void testUpdateEmployeeState() {
    ...
    employeeStateChangeService.updateEmployeeStatus(entity.getUserId(), EmployeeEvent.APPROVE);
    assert(entity.getState().equals(EmployeeState.ADDED));
    assert(!entity.getState().equals(EmployeeState.APPROVED));
    employeeStateChangeService.updateEmployeeStatus(entity.getUserId(), EmployeeEvent.CHECK);
    assert(entity.getState().equals(EmployeeState.IN_CHECK));
    assert(!entity.getState().equals(EmployeeState.ADDED));
    employeeStateChangeService.updateEmployeeStatus(entity.getUserId(), EmployeeEvent.ACTIVATE);
    assert(entity.getState().equals(EmployeeState.IN_CHECK));
    assert(!entity.getState().equals(EmployeeState.ACTIVE));
    employeeStateChangeService.updateEmployeeStatus(entity.getUserId(), EmployeeEvent.APPROVE);
    assert(entity.getState().equals(EmployeeState.APPROVED));
    assert(!entity.getState().equals(EmployeeState.IN_CHECK));
    employeeStateChangeService.updateEmployeeStatus(entity.getUserId(), EmployeeEvent.ADD);
    assert(entity.getState().equals(EmployeeState.APPROVED));
    assert(!entity.getState().equals(EmployeeState.IN_CHECK));
    employeeStateChangeService.updateEmployeeStatus(entity.getUserId(), EmployeeEvent.ACTIVATE);
    assert(entity.getState().equals(EmployeeState.ACTIVE));
